...
func (c Match) New(gameid, groupid int) revel.Result {
	ok, game := models.FindGame(gameid)
	if ok {
		return c.Render(game, groupid)
	}
	return c.RenderText(fmt.Sprintf("Couldn't find a game with that id! (%d)", gameid))
}

// Create creates a match in the database
func (c Match) Create(gameid, groupid int, datePlayed time.Time) revel.Result {
	...
	if c.Validation.HasErrors() {
		c.Validation.Keep()
		c.FlashParams()
		return c.Redirect(Match.New, gameid, groupid)
	}

	if username != nil {
		if ok, match := models.CreateMatch(gameid, groupid, username.(string), datePlayed); ok {
